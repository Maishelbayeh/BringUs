    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg relative shadow-xl border border-primary/20">
        ...
        <div className={`flex ${isRTL ? 'flex-row-reverse' : 'flex-row'} items-center gap-6 mb-6 border-b-2 border-primary/20 pb-4`}>
          <div className={`h-20 w-20 flex items-center justify-center rounded-full text-3xl font-bold shadow ${colors[colorIdx]}`}>{(isRTL ? customer.nameAr : customer.nameEn)[0]}</div>
          <div className={`flex-1 min-w-0 ${isRTL ? 'text-right' : 'text-left'}`}> 
            <div className={`flex items-center gap-2 mb-2 ${isRTL ? 'flex-row-reverse ' : ''}`}>
              <h2 className="text-xl font-bold text-primary">{isRTL ? customer.nameAr : customer.nameEn}</h2>
