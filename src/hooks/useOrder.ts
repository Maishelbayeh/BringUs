...
export function useOrder(storeId: string): UseOrderResult {
  ...
  const fetchOrders = () => {
    setIsLoading(true);
    setError(null);
    fetch(`https://bringus-backend.onrender.com/orders/store/${storeId}`, {
  ...
  const updateOrderPaymentStatus = async (orderId: string, paid: boolean) => {
    try {
      ...
      const paymentResponse = await fetch(`https://bringus-backend.onrender.com/orders/${orderId}/payment-status`, {
        method: 'PUT',
        headers: getAuthHeaders(),
      ...
      if (paymentResult.success) {
        // ثم تحديث حالة الطلب
        const statusResponse = await fetch(`https://bringus-backend.onrender.com/orders/${orderId}/status`, {
          method: 'PUT',
          headers: getAuthHeaders(),
  ...
  const createOrder = async (orderData: CreateOrderData): Promise<CreateOrderResponse> => {
    ...
    try {
      ...
      const response = await fetch(`https://bringus-backend.onrender.com/orders/store/${storeId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const createGuestOrder = async (orderData: CreateOrderData & { guestId: string }): Promise<CreateOrderResponse> => {
    ...
    try {
      ...
      const response = await fetch(`https://bringus-backend.onrender.com/orders/store/${storeId}/guest`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const deleteOrder = async (orderId: string) => {
    try {
      ...
      const response = await fetch(`https://bringus-backend.onrender.com/orders/${orderId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
